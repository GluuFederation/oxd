%define name1 oxd-server-3.1.5
Name:           oxd-server-3.1.5
Version:        1
Release:        1.centos6
...
Source0:        %{name}.tar.gz
Source1:        oxd-server-3.1.5.init.d
Source2:        oxd-https-extension
Source3:        oxd-server-default
BuildArch:      noarch
Conflicts:      oxd-server-3.1.5
...
install -m 644 %SOURCE3 %{buildroot}/etc/default/oxd-server-3.1.5
...
install -m 644 oxd-https-extension/target/oxd-https-extension-3.1.5-SNAPSHOT.jar %{buildroot}/opt/oxd-https-extension/lib/
...
cp -a debian/oxd-server-default %{buildroot}/etc/default/oxd-server-3.1.5

%clean
rm -rf $RPM_BUILD_ROOT

%post
chkconfig --add oxd-server-3.1.5
...
chown root:root /etc/default/oxd-server-3.1.5 2>&1
chown jetty:jetty -R /opt/oxd-server 2>&1
chmod 644 /etc/default/oxd-server-3.1.5 2>&1
mkdir -p /var/log/oxd-server 2>&1
touch /var/log/oxd-server/oxd-server.log 2>&1
chown -R jetty:jetty /var/log/oxd-server 2>&1

%preun
if [ -x /etc/init.d/oxd-server-3.1.5 ] || [ -e /etc/init/oxd-server.conf ]; then
service oxd-server-3.1.5 stop > /dev/null 2>&1
fi
chkconfig --del oxd-server-3.1.5
